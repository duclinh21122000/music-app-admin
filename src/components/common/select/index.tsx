...
import IconCheck2 from '@/components/icons/icon-check'
...
export const Select: SelectComponent = (props) => {
  ...
  const placeholderValue = placeholder || 'Select'
  ...
  const renderSimple = () => {
    return (
      <div className='w-full px-1'>
        ...
        <span
          className={`select-none appearance-none truncate text-primary ${
      ...
      <>
        {arr && arr.length > 0
          ? arr.map((item: OptionsProps) => {
              return renderMultiple ? (
                ...
                <div
                  key={item.value}
                  className='flex w-fit items-center justify-between gap-2 rounded-lg border border-border-200 px-2 py-1 text-primary'
                >
        ...
        {showSearch ? (
          <input
            ...
            disabled={disabled || isLoading}
            className={`h-5 flex-1 p-1 text-primary focus:outline-none`}
            size={10}
            placeholder={
  ...
  const renderOption = () => {
    ...
    return arrMap
      ? arrMap.map((opt) => {
          ...
          const createLabel = `Label "${searchKey}"`
          ...
          return (
            ...
            >
              {renderOptions ? (
                ...
                <span className='truncate text-primary'>{opt.label}</span>
              )}
              {selected && multiple ? (
                <IconCheck2 width={18} color='var(--primary-500)' />
              ) : null}
            </li>
          )
        })
